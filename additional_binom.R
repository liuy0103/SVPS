...
A_sim <- function(M, n_binom = 5){
  M[M >= 1] = 1
  A_temp = matrix(0, n, n)
  upperTriangle(A_temp) = rbinom(n*(n-1)/2, size = n_binom, upperTriangle(M)/n_binom)
  A = A_temp + t(A_temp)
  diag(A) = rbinom(n, n_binom, diag(M)/n_binom)
  return(A)
}
...
n_binom <- 5 # Number of trials in the binomial distribution
...
# n_vec_whole <- c(50, 150, 50, 150, 50, 150, 50, 150, 50, 150)
# n_vec_whole <- c(50, 100, 150, 50, 100, 150, 50, 100, 150)
...
for(K in K_vec){
  ...
  rho_K_hat <- cbind(rho_K_hat, c(sum(K_hat_vec == K), sum(bic_vec == K), sum(icl_vec == K), 
                                  sum(K_hat_vec_2 == K), sum(bic_vec_2 == K), sum(icl_vec_2 == K)
                                  # , sum(mod_vec == K), sum(mod_vec_2 == K) 
  ) / ite_max)
}

all_methods <- c('SVPS_SCORE', 'CBIC_SCORE', 'ICL_SCORE', 'SVPS_RSC', 'CBIC_RSC',
                 'ICL_RSC', 'Modularity_SCORE', 'Modularity_RSC')
all_plot_color <- c("blue","red","green","black","orange","purple")
all_plot_shape <- c(0, 1, 2, 3, 4, 5, 6, 7)
all_plot_line <- c('solid','twodash','12345678','longdash','dashed','dotted')

len_method <- 6

plot_data <- data.frame(t(rho_K_hat))
colnames(plot_data) <- all_methods[1:len_method]
...
ggplot(df, aes(x = K, y = value, linetype = variable, shape = variable, color = variable)) +
  ...
  scale_colour_manual(name = element_blank(),
                      labels = all_methods[1:len_method],
                      values = all_plot_color[1:len_method]
  ) +
  scale_shape_manual(name = element_blank(),
                     labels = all_methods[1:len_method],
                     values = all_plot_shape[1:len_method]
  ) +
  scale_linetype_manual(name = element_blank(),
                        labels = all_methods[1:len_method],
                        values = all_plot_line[1:len_method]
  ) +
