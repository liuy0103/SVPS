...
CBIC_ICL <- function(A, Kmax = 10, clus = 'SCORE', lambda = 1, tau = 0.25) 
{
  ...
  for (m in 1:Kmax) {
    ...
    P_hat = pmin(1-delta, M_hat/n_binom)
    loglike = sum(upperTriangle(A*log(P_hat) + (n_binom-A)*log(1-P_hat), diag = T))
...
Stepwise <- function(A, Kmax = 15, clus = 'SCORE', thres, tau = 0.25){
  d_i <- rowSums(A)
  n <- dim(A)[1]
...
r_val <- 2 # Unbalancedness, which is r in Section 5.1
rho <- 0.12 # Sparsity, which is rho in Section 5.1
...
ggsave("plots_new/binom_r=2.png", width = 200, height = 140, units = 'mm')

save.image(file = "workspace_new/binom_r=2.RData")
